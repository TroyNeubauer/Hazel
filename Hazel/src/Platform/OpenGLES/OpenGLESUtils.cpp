...
namespace Hazel {
	...
	const char* OpenGLESUtils::GetGLShaderTypeString(GLenum type)
	{
		switch (type)
		{
	...
	GLenum OpenGLESUtils::ShaderDataTypeToGLType(ShaderDataType type) {
		switch (type)
		{
	...
	GLenum OpenGLESUtils::TextureFormatTGLType(TextureFormat format)
	{
		switch (format)
		{
	...
	GLuint OpenGLESUtils::Upload2DTexture(File* file, uint32_t& width, uint32_t& height, TextureBuilder builder)
	{
		HZ_PROFILE_FUNCTION();
...

		{
			...
			if ((FreeImage_GetBits(dib) == nullptr) || (width == 0) || (height == 0))
				return false;
			switch (type)
			{
			...
			id = Upload2DTexture(width, height, FreeImage_GetBits(dib), imageFormat, imageType, builder);
		}
	end:
	...
	GLuint OpenGLESUtils::Upload2DTexture(uint32_t width, uint32_t height, void* data, TextureFormat format, TextureBuilder builder)
	{
		return Upload2DTexture(width, height, data, TextureFormatTGLType(format), GL_UNSIGNED_BYTE, builder);
	}

	GLuint OpenGLESUtils::Upload2DTexture(uint32_t width, uint32_t height, void* data, int imageFormat, int imageType, TextureBuilder builder)
	{
		HZ_PROFILE_FUNCTION();
